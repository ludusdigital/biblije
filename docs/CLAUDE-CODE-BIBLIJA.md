...
Gotovo je kada: stranica se otvara u browseru bez grešaka,
forma odbija prazna polja, i pošalješ mi screenshot kako izgleda
...
| 6 | **Ritam razvoja** — ciklus feature-po-feature, formula savršenog prompta |
...
| 9 | **Lansiranje** — Vercel, env varijable, domen, pre-launch checklist |
...
| Dodaci | Cheat-sheet komandi, šabloni promptova, rečnik pojmova |
| Za predavače | Kako iz dokumenta izvesti kurs: radionice, kapstone projekat |
...
**Pravilo 1: Merljivost.** Sve što tražiš mora imati proverljiv kriterijum gotovosti. „Napravi da bude lepo" niko ne može da proveri — ni Claude, ni ti. „Forma odbija prazan email i prikazuje poruku o grešci" može da se proveri za deset sekundi. Pre nego što pošalješ zahtev, zapitaj se: *kako ću znati da je ovo gotovo?* Ako nemaš odgovor, zahtev nije spreman.
...
**Pravilo 3: Dokaz umesto poverenja.** Kada Claude kaže „gotovo je, sve radi" — to je tvrdnja, ne dokaz. Direktor ne potpisuje na reč. Uvek traži dokaz: rezultat testa, screenshot stranice, log (zapis onoga što se zaista dogodilo pri izvršavanju). Claude ume da pokrene aplikaciju, klikće po njoj i napravi screenshot — iskoristi to. Rečenica koju ćeš najčešće dodavati na kraj svojih zahteva glasi:

```
Kad završiš, dokaži mi da radi: pokreni testove i pošalji rezultat,
i napravi screenshot stranice u browseru.
```
...
```
Napravi React komponentu sa useState hookom koja preko fetch API-ja
...
(3) pokažeš mi screenshot obe situacije.
```
...
- **screenshot** ekrana na kom se vidi da funkcija radi,
- **log** — ispis programa koji pokazuje šta se stvarno dogodilo.

Claude Code može sam da pokrene tvoju aplikaciju i proveri je u browseru: da klikće, popunjava forme i napravi screenshot. Iskoristi to — umesto „jesi li siguran da radi?", traži:

```
Pokreni aplikaciju, prođi kroz prijavu kao novi korisnik i pokaži mi
screenshot ekrana posle uspešne prijave. Zatim pokreni testove i
pokaži mi ispis.
```
...
4. **Prihvatanje „gotovo je" bez dokaza.** Rešenje: uvek traži test, screenshot ili log. Bez dokaza, posao nije završen.
...
- [ ] Ne prihvatam „gotovo je" bez testa, screenshota ili loga
...
Brzo ćeš primetiti da Claude pre određenih akcija traži tvoje odobrenje: „Mogu li da pokrenem ovu komandu?", „Mogu li da izmenim ovaj fajl?". To nije znak da je nesiguran — to je sigurnosni sistem. Claude pita pre potencijalno opasnih akcija, da slučajno ne obriše ili izmeni nešto bez tvog znanja. Kao novi zaposleni koji proverava sa šefom pre nego što pošalje mejl klijentu.
...
- Formulu za donošenje tehničkih odluka bez tehničkog znanja: 2 opcije + preporuka + trade-off
...
- Standardni početni stack (Next.js + Supabase + Vercel) objašnjen jezikom prodavnice, a ne programera
...
### Formula odlučivanja: 2 opcije + preporuka + trade-off
...
3. Objasni trade-off (šta dobijam, šta žrtvujem) jezikom za nekoga
   ko nije programer — bez žargona, sa poređenjem iz svakodnevnog života.
```

*Trade-off* je razmena: svaki izbor nešto daje i nešto uzima. Jeftiniji lokal je dalje od centra. Brži auto troši više. Ne postoji opcija bez cene — postoji samo cena koju nisi video. Tvoj posao nije da znaš tehnologije; tvoj posao je da tražiš da ti cena bude pokazana.
...
### Standardni početni stack: izlog, magacin i lokal

*Stack* je skup tehnologija od kojih je proizvod sagrađen — kao spisak materijala za kuću. Za prvi proizvod ne moraš da istražuješ: postoji proverena kombinacija koju koristi ogroman broj malih timova, pa je i Claude odlično poznaje.
...
preporuku i trade-off objašnjen laički.
```
...
5. **Egzotičan stack jer je "najnoviji".** Za novu tehnologiju ima manje primera, pa i Claude i ti češće lutate. Rešenje: standardni stack dok ne postoji konkretan, laički objašnjen razlog za izuzetak.

### Kontrolna lista

- [ ] Znam da uključim plan mode (`Shift+Tab` ili rečenicom) i kad da ga koristim
- [ ] Svaku važnu odluku tražim kao: 2 opcije + preporuka + trade-off laički
...
## Modul 6: Ritam razvoja — feature po feature

Stigao si do srca cele metodologije. Sve do sada — okruženje, temelji, specifikacija, plan — bila je priprema. Odavde pa do lansiranja, 80% tvog vremena provodi se u jednom istom ciklusu koji se ponavlja za svaki feature (funkcionalnost — jednu zaokruženu stvar koju proizvod radi, npr. "kontakt forma" ili "prijava korisnika"). Kad taj ciklus uđe u ruke, razvoj prestaje da bude haos i postaje zanat: predvidiv, miran, merljiv.

Zamisli ga kao pranje veša. Ne ubacuješ sve odjednom u mašinu — sortiraš, opereš jednu turu, proveriš da li je čisto, složiš, pa tek onda sledeća tura. Jedan feature = jedna tura.

### Šta ćeš naučiti

- Ciklus od 7 koraka koji ponavljaš za svaki feature, od prazne sesije do potvrde da sve radi
- Zašto svaki feature zaslužuje svežu sesiju (`/clear`) i šta se dešava kad to preskočiš
- Kako da tražiš dokaz umesto obećanja — screenshot, test, log
...
**Korak 1: `/clear` — sveža sesija za svaki feature.** Sesija je jedan razgovor sa Claude-om, a kontekst je njegova radna memorija u tom razgovoru (detaljno u Modulu 1). Kad u istoj sesiji nakupiš tri feature-a, dve ispravke i jedan eksperiment, ta memorija se zatrpa — Claude počinje da meša stare zahteve sa novima, kao konobar koji pamti porudžbine celog dana umesto samo tvog stola. Zato: novi feature, nova sesija. Ne brini za "zaboravljanje" — sve trajno važno živi u `CLAUDE.md` i u tvojim fajlovima sa specifikacijom, a oni se učitavaju iznova.

**Korak 2: Plan mode za sve što nije trivijalno.** Plan mode je režim u kom Claude istražuje projekat i predlaže plan, ali ne menja nijedan fajl dok ti plan ne odobriš. Uključuješ ga prečicom `Shift+Tab` ili jednostavno napišeš: "prvo napravi plan, ne menjaj ništa dok ne odobrim". Pravilo palca: ako feature dira više od jednog fajla ili uvodi nešto novo (bazu, plaćanje, slanje mejlova) — plan mode. Ako je sitnica ("promeni boju dugmeta") — preskoči. Kako da čitaš i izazivaš predloženi plan naučio si u Modulu 5.
...
**Korak 4: Verifikacija u browseru — traži dokaz.** Ovo je korak koji početnici najčešće preskaču, a najviše vredi. Claude ume da pokrene dev server (lokalnu, probnu verziju tvoje aplikacije na tvom računaru), klikće po njoj, popunjava forme i pravi screenshotove. Nikad ne prihvataj "implementirano je" kao kraj — to je obećanje, ne dokaz.

```
Pokreni dev server, otvori stranicu /kontakt u browseru, popuni formu
probnim podacima i pošalji je. Napravi screenshot forme pre slanja i
screenshot poruke o uspehu posle slanja. Pokaži mi oba.
```
...
**Korak 6: Commit.** Commit je snimak stanja projekta u git-u — kao "sačuvaj igru" u video igrici: tačka kojoj uvek možeš da se vratiš (osnove git-a su u Modulu 3). Jedan feature = jedan commit (ili nekoliko manjih, ali nikad "sve od prošle nedelje odjednom").

```
Napravi commit ovih izmena sa jasnom porukom na srpskom koja opisuje
šta feature radi, i pošalji na GitHub.
```
...
I onda? `/clear` i sledeći feature. To je ceo ritam.
...
**2. ZADATAK — šta se pravi.** Jedna jasna rečenica, jedan feature. Ako u zadatku stoji veznik "i" između dve različite stvari, verovatno su to dva ciklusa.
...
✅ `Gotovo je kad: forma na /kontakt prima ime, email i poruku; posle slanja poruka se vidi u bazi; korisnik dobije potvrdu na ekranu; npm run build prolazi bez grešaka.`
...
Prođimo ceo ciklus na stvarnom feature-u, prompt po prompt.
...
formu na stranici /kontakt: polja ime, email i poruka, čuvanje poruka
...
**3.** Pročitaj plan, postavi pitanja ("šta se desi ako neko pošalje prazan email?"), odobri. Claude implementira.

**4.** Dokaz:

```
Pokreni dev server i verifikuj u browseru: popuni formu na /kontakt
podacima "Marko Marković, marko@test.com, Probna poruka" i pošalji.
Napravi screenshot potvrde. Zatim mi pokaži da ta poruka stvarno
postoji u bazi. Probaj i slanje prazne forme — screenshot poruke
o grešci.
```

Pogledaj screenshotove svojim očima. Ti si poslednja kontrola.
...
1. Otvori svoj `docs/spec.md` (napravio si ga u Modulu 4) i izaberi najmanji nezapočeti feature sa liste.
...
4. Prođi svih 7 koraka do kraja, uključujući screenshot kao dokaz i commit.
5. Na kraju zapiši: koji korak ti je bio najneprirodniji? Njega sledeći put radi prvo.

### Najčešće greške

1. **Nastavljanje u istoj sesiji ceo dan.** Simptom: Claude "meša" stare i nove zahteve, vraća izmene koje si već odbacio. Rešenje: `/clear` posle svakog završenog feature-a; ako sesija mora da bude duga, `/compact` da sažme razgovor.
2. **Prihvatanje "gotovo je" bez dokaza.** Claude može iskreno da veruje da nešto radi — i da greši. Rešenje: uvek traži screenshot, izlaz testova ili log; korak 4 nije opcioni.
...
4. **Tri feature-a u jednom promptu.** Plan postane ogroman, verifikacija nemoguća, a kad nešto pukne ne znaš šta je krivac. Rešenje: jedan ciklus = jedan feature; ostalo ide u backlog (listu čekanja, Modul 10).
...
- [ ] Počeo sam feature sa `/clear`, u svežoj sesiji
- [ ] Za netrivijalan feature koristio sam plan mode i odobrio plan pre izmena
- [ ] Moj prompt ima sva četiri dela: kontekst, zadatak, kriterijum prihvatanja, ograničenja
- [ ] Video sam dokaz da feature radi (screenshot, test, log) — ne samo tvrdnju
...
**1. Zašto se svaki feature radi u svežoj sesiji?**
...
**3. Šta je prihvatljiv dokaz da feature radi, a šta nije?**
Prihvatljivo: screenshot iz browsera, izlaz testova, zapis u bazi, log. Neprihvatljivo: rečenica "implementirano je" — to je obećanje, a ti tražiš dokaz koji možeš da vidiš svojim očima.
...
| Stranica `/kontakt` postoji i screenshot pokazuje formu sa poljima za ime, mejl i poruku | ✅ | Dokaz je slika, sadržaj je nabrojan stavku po stavku |
...
### /schedule: noćna smena u cloudu

Kad ti treba rad i kad je računar ugašen, tu je `/schedule` — Routines: zakazani agenti koji rade u cloudu, ne na tvojoj mašini. Klasičan primer: dnevni izveštaj o stanju projekta koji te čeka uz jutarnju kafu. Za automatizaciju vezanu za repo (npr. pregled svakog PR-a) postoji i GitHub Actions. Više o rutinama posle lansiranja u Modulu 10.
...
| Automatizacija vezana za repo (pregled svakog PR-a) | GitHub Actions |
...
Mali brzi model (Haiku) posle svake ture ocenjuje da li uslov važi — ali vidi samo ono što Claude pokaže u razgovoru. Zato uslov mora biti dokaziv: izlaz komande, log ili screenshot, ne utisak.
...
`/schedule` — zakazani cloud agenti rade i kad je tvoj računar ugašen. `/goal` i `/loop` žive unutar otvorene sesije na tvojoj mašini, pa sa ugašenim računarom ne rade.
...
Claude može da pokrene dev server i proveri ovo u browseru — da klikće, popunjava forme i pravi screenshotove. Ako "napad" uspe, imaćeš screenshot ili log kao dokaz, a onda i jasan zadatak za ispravku.

### Princip dokaza i crvene zastavice

Iz Modula 1 znaš pravilo: **tvrdnja bez dokaza se ne računa.** "Popravio sam" ne znači ništa bez test outputa, screenshota ili loga. U kontekstu kvaliteta, ovo su tri crvene zastavice na koje reaguješ odmah:

❌ **"Gotovo je" bez dokaza.** Odgovor: "Pokaži mi output testova / screenshot ekrana / log koji to potvrđuje."
...
✅ Zdrav obrazac izgleda ovako: izmena → testovi prolaze (vidiš output) → `/code-review` → nalazi prevedeni i odlučeni → dokaz da ispravke rade.
...
7. Za jednu odobrenu ispravku traži dokaz da radi: test output ili screenshot.
...
4. **Prihvatanje "popravio sam testove" bez gledanja.** Možda su obrisani. *Rešenje:* traži da ti pokaže šta je tačno promenjeno i output testova pre i posle.
...
- [ ] Ne prihvatam "gotovo je" bez test outputa, screenshota ili loga
...
Medium za svakodnevni ritam posle svake funkcionalnosti — brz, fokusiran na najvažnije. Ultra za najvažnije prekretnice (pred lansiranje, posle velikog refaktorisanja) — multi-agent pregled celog brancha u cloudu, najtemeljniji, dodatno se naplaćuje i pokrećeš ga ti.
...
Dokaz: output testova i pregled šta je tačno izmenjeno. Posebno proveravaš da nijedan test nije obrisan ili oslabljen — tvrdnja bez dokaza se ne računa.
...
- Zašto svaki novi feature i posle lansiranja prolazi isti disciplinovani ciklus iz Modula 6
...
`/schedule` pravi **Routines** — zakazane agente koji rade u cloudu (na tuđim serverima, ne na tvom računaru), pa funkcionišu i kad je tvoj laptop u rancu. To je kao da si zaposlio noćnog čuvara.
...
Korisnici će ti pisati: mejlom, porukom, usmeno na kafi. Svaka takva poruka je zlato, ali zlato koje se lako zaturi. Zato uvedi jedno mesto za sve: `docs/backlog.md`. **Backlog** je spisak svega što bi proizvod jednog dana mogao da dobije — spisak želja, ne spisak obaveza.
...
Odnos vrednost/trud je tvoj kompas: feature koji usreći mnogo korisnika a traži dan posla pobeđuje feature koji usreći jednog a traži mesec. Claude procenjuje trud bolje od tebe (vidi kod iznutra), ti procenjuješ vrednost bolje od njega (poznaješ korisnike). Zajedno ste kompletan tim.
...
Svaki novi feature, ma koliko sitan, prolazi isti ciklus iz Modula 6: plan mode pre koda, mali koraci, dokaz da radi (test, screenshot, log — princip iz Modula 1), pa `/code-review` i `/security-review` pre deploy-a (Modul 8). Razlika: na kraju ciklusa sada stoji deploy u produkciju — i `/loop` dežurstvo posle njega.

### Refaktorisanje: sređivanje magacina

Kako proizvod raste, kod se prirodno zapetljava — kao magacin u koji mesecima samo ubacuješ robu. Jednog dana ni ti ni Claude više ništa ne nalazite brzo, a svaki novi feature traje duplo duže.

**Refaktorisanje** je sređivanje tog magacina: kod se reorganizuje da bude pregledniji i lakši za dalji rad, a proizvod spolja radi potpuno isto. Korisnici ne vide razliku — ali svaki sledeći feature stiže brže.
...
5. **Odlaganje refaktorisanja dok ne postane kriza.** Kad svaki novi feature traje duplo duže, magacin je već zatrpan do plafona. Rešenje: jednom mesečno pitaj Claude-a da li se neki deo koda zapetljao i tretiraj sređivanje kao ravnopravnu stavku backloga.
...
- [ ] Svaki novi feature prolazi pun ciklus iz Modula 6, bez prečica „jer je sitno"
...
1. `/loop` ponavlja zadatak unutar otvorene sesije — radi samo dok je sesija živa i računar upaljen. `/schedule` pravi rutine koje rade u cloudu, po rasporedu, i kad je tvoj računar ugašen. Dežurstvo posle deploy-a = `/loop`; trajne provere = `/schedule`.
...
- **6 radionica po 2 do 2,5 sata**, jednom nedeljno. Nedeljni razmak nije slučajan: polaznicima treba vreme između radionica da rade na kapstone projektu i da se sapletu — jer se na sapletanju uči.
- **Uživo ili online** — oba formata rade. Online zahteva da svaki polaznik deli ekran kad zapne; uživo zahteva da ti hodaš po sali i gledaš ekrane. U oba slučaja, predavač mora da vidi šta polaznik radi, ne samo da čuje šta kaže.
- **Kapstone projekat** se proteže kroz ceo kurs: svaki polaznik bira svoj mali proizvod na drugoj radionici i lansira ga do šeste. Kurs bez kapstona je predavanje; kurs sa kapstonom je transformacija.
...
| R2 | Moduli 3–4 | Temelji (git, GitHub, `/init`) + specifikacija: **svako napiše `docs/spec.md` svoje kapstone ideje** |
| R3 | Moduli 5–6 | Plan mode + ritam razvoja: **svako izgradi prvi feature uživo**, kroz ciklus plan → izvršenje → dokaz → commit |
...
| R6 | Moduli 11–13 | Majstorstvo promptovanja, ekosistem, dijagnostika problema + **prezentacije kapstone projekata** |
...
3. **Samostalni zadatak (40 min)** — svako radi na svom kapstone projektu, primenjujući ono iz vežbe. Ti kružiš i odglavljuješ. Ovde se gradivo pretvara u veštinu.
...
Svaki polaznik na R2 bira **mali, stvarni proizvod**: jedan problem, jedna ciljna grupa, jedna stvar koju aplikacija radi. Ne „platforma za fitnes" nego „stranica gde moji klijenti zakazuju termin i dobiju potvrdu mejlom". Pravilo iz Modula 4 ovde sprovedi nemilosrdno: ako spec ne staje na jednu stranu, projekat je prevelik za šest nedelja.
...
- [ ] Bar jedan feature je izgrađen kroz ciklus: plan mode → odobrenje → implementacija → dokaz (test ili screenshot) → commit
...
3. **Dozvole.** Kad Claude prvi put pita „da li smem da pokrenem ovu komandu?", polaznici se uplaše da su nešto pokvarili. Objasni unapred: to je sigurnosna provera, kao kad ti novi saradnik kaže „hoću da pošaljem ovaj mejl klijentu, jesi saglasan?" — znak da sistem radi, ne da je nešto pošlo naopako. Pokaži kako odobravanje izgleda, šta znači lista dozvoljenih komandi u `.claude/settings.json`, i pomeni da skill `/fewer-permission-prompts` ume da predloži bezbednu listu (detaljno u Modulu 7).

**Namerno izazovi grešku uživo.** Ovo je najjači trik u tvom arsenalu. Negde u demou — najbolje na R3 — namerno traži nešto dvosmisleno ili pusti da build pukne, pa pred svima prođi kroz oporavak: pročitaj poruku o grešci naglas, prevedi je na običan jezik, nalepi je Claude-u i traži dijagnozu, pritisni `Esc` ako je krenuo pogrešnim putem i preusmeri ga. Polaznici koji su videli predavača kako mirno izlazi iz greške ne paniče kad se njima desi — a desiće se iste večeri. Ako greška neće da se desi sama, izazovi je: traži feature bez ikakvog konteksta („dodaj plaćanje") i pokaži zašto je rezultat loš, pa isti zahtev formuliši po formuli iz Modula 6 i pokaži razliku.

**Koristi šablone iz Dodatka B kao radne listove.** Ne teraj polaznike da promptove smišljaju iz glave dok su početnici — podeli šablone iz Dodatka B (odštampane ili kao fajl) i neka u samostalnom delu radionice popunjavaju praznine svojim projektom. Šablon za spec na R2, šablon za feature na R3, šablon za `/goal` uslov na R4, pre-launch checklist na R5. Vremenom će šablone odbaciti sami — ali u prvim nedeljama, popunjavanje praznina pretvara paralizu pred praznim ekranom u rutinu.

I poslednji savet: vodi sopstveni kapstone paralelno sa polaznicima. Predavač koji svake nedelje pokaže svoj napredak, svoje greške i svoje commit-ove nije autoritet koji priča o plivanju sa obale — on je u vodi, pored njih. To je ton celog ovog dokumenta, i ton koji kurs čini uspešnim.
